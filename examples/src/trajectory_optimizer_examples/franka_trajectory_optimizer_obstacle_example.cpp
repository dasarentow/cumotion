...
//  2. Create world environment with cuboid obstacle.
...
int main() {
  ...
  // A `World` represents a collection of obstacles that the robot must avoid during trajectory
  // planning. This provides the environment context for collision-aware trajectory optimization.
  auto world = cumotion::CreateWorld();

  // Create a cuboid obstacle.
  const auto obstacle = cumotion::CreateObstacle(cumotion::Obstacle::Type::CUBOID);

  // Set the side lengths for the cuboid obstacle.
  const Eigen::Vector3d cuboid_side_lengths(0.2, 0.2, 0.15);
  obstacle->setAttribute(cumotion::Obstacle::Attribute::SIDE_LENGTHS, cuboid_side_lengths);
  ...
  const Eigen::Vector3d cuboid_position(0.3, 0.15, 0.3);
  const cumotion::Rotation3 cuboid_orientation =
      cumotion::Rotation3::FromAxisAngle(Eigen::Vector3d::UnitZ(), M_PI / 3.0);
  const cumotion::Pose3 cuboid_pose(cuboid_orientation, cuboid_position);

  // Add the configured obstacle to the world at the specified pose.
  world->addObstacle(*obstacle, cuboid_pose);
  ...
  // trajectory optimization. Upon creation, the `world_view` contains the latest snapshot of
  // `world` with all presently enabled obstacles.
